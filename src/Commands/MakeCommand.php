...
class MakeCommand extends Command implements PromptsForMissingInput
{
    ...
    protected string $relativeClassPath;
    ...
    {
        $this->resolveClassName($this->argument('name'));
    ...
    /**
     * Resolve the target class name, namespace, and path from the given name.
     *
     * Replaces Livewire 3's ComponentParser (removed in Livewire 4) with manual
     * resolution against the livewire config.
     */
    protected function resolveClassName(string $name): void
    {
        ...
        $this->className = (string) array_pop($segments);
        ...
        $subPath = ! empty($segments) ? implode(DIRECTORY_SEPARATOR, $segments).DIRECTORY_SEPARATOR : '';
        $this->classPath = rtrim($classBasePath, '/\\').DIRECTORY_SEPARATOR.$subPath.$this->className.'.php';
        $this->relativeClassPath = Str::of($this->classPath)
    ...
    protected function createClass(bool $force = false): bool
    {
        ...
        if (! $force && File::exists($classPath)) {
            $this->line("<fg=red;options=bold>Class already exists:</> {$this->relativeClassPath}");
